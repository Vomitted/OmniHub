...
public sealed class PowerTransitionLog : IDisposable
{
    ...
    // Overridable only so a test can prove the flush-per-row promise without writing invented
    // rows into the real file. That file is read back after a hang to reconstruct what happened,
    // and a test fixture appearing in it would corrupt the one record this class exists to keep
    // honest.
    private readonly string _directory;

    public PowerTransitionLog(string? directory = null) => _directory = directory ?? LogDirectory;
    ...
    private void EnsureWriterFor(DateTime utcNow)
    {
        ...
        string path = Path.Combine(_directory, $"power-{utcNow:yyyy-MM-dd}.csv");
